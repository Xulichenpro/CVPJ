...
def eval_func_chunked(
    ...
    for start in range(0, num_q, query_chunk_size):
        ...
        if start == 0 or start // query_chunk_size % 100 == 0:
            print("=> Chunked eval query [{}/{}]".format(start, num_q))
        distmat = euclidean_distance(qf[start:end], gf)
        indices = np.argsort(distmat, axis=1)

        for local_q_idx, order in enumerate(indices):
...
class R1_mAP_eval():
    ...
    def compute(self):  # called after each epoch
        ...
        if self.reranking:
            ...
            cmc, mAP = eval_func(distmat, q_pids, g_pids, q_camids, g_camids, dataset_name=self.dataset_name)

        else:
            print('=> Computing DistMat with euclidean_distance')
            if str(self.dataset_name).lower() == "ccvid":
                print('=> Computing metrics with chunked euclidean_distance')
                cmc, mAP = eval_func_chunked(
                    ...
                    query_chunk_size=os.environ.get("CLIP_REID_EVAL_QUERY_CHUNK", 64),
                )
                distmat = None
            else:
